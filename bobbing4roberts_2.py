...
def generate_usernames_by_format(first, last, formats):
    formats_mapping = {
        1: f"{first[0].lower()}{last.lower()}",  # fLast
        2: f"{first.lower()}.{last.lower()}",  # first.last
        3: f"{first.lower()}{last[0].lower()}",  # firstL
        4: f"{first.lower()}",  # first only
        5: f"{last.lower()}",  # last only
        6: f"{last.lower()}{first[0].lower()}"  # last + f
    }
    
    # Handle the new f.last format explicitly as a string format
    if 'f.last' in formats:
        formats_mapping['f.last'] = f"{first[0].lower()}.{last.lower()}"
    
    selected_formats = [formats_mapping[f] for f in formats.split(',') if f in formats_mapping]
...
def print_help():
    ...
    print('Usage: script.py -i <inputfile> -o <outputfile> [-a <additionalfile> | -s <swapfile>] [-f <format numbers or f.last>]')
    print('\nOptions:')
    ...
    print('  f.last: j.smith       First initial + Last name with a dot')
    ...
    print('  Specify f.last format only: script.py -i test.txt -o out.txt -f f.last')
...
def main(argv):
    ...
    default_namePair = os.path.join(os.path.dirname(os.path.abspath(__file__)), "namePairs.txt")
    if swap_namePair:
